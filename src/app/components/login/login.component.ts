	...
	loginObject(){
        ...
        this.userSvc.login(this.model.email, md5(this.model.password), sessionKey)
            .then((result) => {
                ...
                if(result.data){
                    this.loading = false;
                    this.userSvc.addItem(result.data.user.name,result.data.user.id,result.data.user.address,result.data.user.creditcardtype, result.data.usersessionKey);
                    this.notifyMessages.push({ type: "confirm", message: "Successful login!! use "+ result.data.user.email +" as username"  });
                }
                else {
                    this.loading = false;
		            this.notifyMessages.push({ type: "error", message: result.data.usermessage || result.data.user|| "Internal server error"  });
                } 
